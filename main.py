...
async def warmup_peers():
    ...
    # Populate Pyrogram peer storage from the account dialogs first.
    try:
        count = 0
        async for dialog in userbot.get_dialogs():
            ...
            if count >= 5000:
                break
        log.info(f"Peer cache warmed from {count} dialogs")
    except Exception as e:
        log.warning(f"Dialog peer warmup failed: {e}")
